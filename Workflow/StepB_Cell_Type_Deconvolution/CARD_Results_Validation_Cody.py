"""
STPath-COAD: CARD Results Validation Analysis
============================================

This script validates CARD deconvolution results by comparing with known cell type
distributions and performs statistical analysis of cell type proportions.

Author: Saishi Cui
Date: Sept 2025

Purpose: Validate CARD deconvolution results, perform statistical comparisons,
and generate validation plots for cell type proportion estimates in the 
STPath-COAD pipeline.
"""
...
for file in CARD_results_celltype_proportion_path_list:
    print(file)
...
def CTProportion_vs_Relative_expression_Visualization(cell_type, marker_genes_dict=final_ranked_gene_dict):
    ...
    for Region_ID in Region_ID_list:
        ...
        plt.savefig(f"data/CARD_Results_Figures/{cell_type}_{Region_ID}.png", dpi=600, bbox_inches='tight')
        plt.close()
...
result_df_Cancer_Cody = CTProportion_vs_Relative_expression_Visualization(cell_type="Cancer Cells")
result_df_Normal_Epith_Cody = CTProportion_vs_Relative_expression_Visualization(cell_type="Normal Epithelial Cells")
result_df_T_Cody = CTProportion_vs_Relative_expression_Visualization(cell_type="T Cells")
result_df_Other_Immune_Cody = CTProportion_vs_Relative_expression_Visualization(cell_type="Other Immune Cells")
result_df_Stromal_Cody = CTProportion_vs_Relative_expression_Visualization(cell_type="Stromal Cells")


result_df_Cody = pd.concat([result_df_Cancer_Cody, result_df_Normal_Epith_Cody, result_df_T_Cody, result_df_Other_Immune_Cody, result_df_Stromal_Cody], axis=0)
result_df_Cody.reset_index(drop=True, inplace=True)
result_df_Cody.to_csv("data/CARD_Results_Files/CARD_Results_Validation_Cody.csv", index=False)
