"""
STPath-COAD: CARD Results Validation for HEST Dataset
====================================================

This script validates CARD deconvolution results specifically for the HEST-1K
dataset and performs statistical analysis of cell type proportions.

Author: Saishi Cui
Date: December 2025

Purpose: Validate CARD deconvolution results on HEST dataset, perform statistical
comparisons, and generate validation plots for cell type proportion estimates
in the HEST-1K histopathology dataset.
"""
...
def CTProportion_vs_Relative_expression_Visualization(cell_type, marker_genes_dict=final_ranked_gene_dict):
    ...
    for Region_ID in Region_ID_list:
        ...
        if len(set(focus_celltype_percent)) <= 1:
            # If all x values are the same, only plot scatter points without regression analysis
            ...
            # Add Spearman correlation annotation - only display when calculable
        ...
        # Adjust tick style - enlarge, darken and bold
        ...
        plt.savefig(f"data/hest_data/CARD_Results_Figures/{cell_type}_{Region_ID}.png", dpi=600, bbox_inches='tight')
        plt.close()
...
result_df_Cancer_HEST = CTProportion_vs_Relative_expression_Visualization(cell_type="Cancer Cells")
result_df_Normal_Epith_HEST = CTProportion_vs_Relative_expression_Visualization(cell_type="Normal Epithelial Cells")
result_df_T_HEST = CTProportion_vs_Relative_expression_Visualization(cell_type="T Cells")
result_df_Other_Immune_HEST = CTProportion_vs_Relative_expression_Visualization(cell_type="Other Immune Cells")
result_df_Stromal_HEST = CTProportion_vs_Relative_expression_Visualization(cell_type="Stromal Cells")


result_df_HEST = pd.concat([result_df_Cancer_HEST, result_df_Normal_Epith_HEST, result_df_T_HEST, result_df_Other_Immune_HEST, result_df_Stromal_HEST], axis=0)
result_df_HEST.reset_index(drop=True, inplace=True)
result_df_HEST.to_csv("data/hest_data/CARD_Results_Files/CARD_Results_Validation_HEST.csv", index=False)
...
Cancer_combined = pd.concat([Cancer_FH, Cancer_Cody, Cancer_HEST], axis=0)
Stromal_combined = pd.concat([Stromal_FH, Stromal_Cody, Stromal_HEST], axis=0)
Normal_Epith_combined = pd.concat([Normal_Epith_FH, Normal_Epith_Cody, Normal_Epith_HEST], axis=0)
Other_Immune_combined = pd.concat([Other_Immune_FH, Other_Immune_Cody, Other_Immune_HEST], axis=0)
T_combined = pd.concat([T_FH, T_Cody, T_HEST], axis=0)
